...
export class CelebrityMatchService {
  // Celebrity database with birth dates, professions, and gender
  private static readonly CELEBRITY_DATABASE = [
    ...
    {
      name: "Ryan Reynolds",
      birthDate: "10/23/1976",
      profession: "Actor",
      lifePathNumber: 3,
    },
    {
      name: "Blake Lively",
      birthDate: "08/25/1987",
      profession: "Actress",
      lifePathNumber: 4,
    },
    {
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              console.log(
                `Rate limited for ${celebrity.name}, using fallback reason`
              );
              matchReason = this.generateFallbackMatchReason(
                userName,
                userLifePath,
                celebrity.name,
                celebrity.lifePathNumber,
  ...
  // Find exactly one male and one female celebrity match for optimized performance
  static async findCelebrityMatchesByGender(
    userLifePath: number,
    userDestiny: number,
    userSoulUrge: number,
    userName: string
  ): Promise<CelebrityMatch[]> {
    // Define male and female celebrities with their gender
    ...
    // Filter and find best male celebrity
    const compatibleMales = this.CELEBRITY_DATABASE.filter(
      (celebrity) =>
        compatibleLifePaths.includes(celebrity.lifePathNumber) &&
        maleCelebrities.includes(celebrity.name)
    );

    // Filter and find best female celebrity
    const compatibleFemales = this.CELEBRITY_DATABASE.filter(
      (celebrity) =>
        compatibleLifePaths.includes(celebrity.lifePathNumber) &&
        femaleCelebrities.includes(celebrity.name)
    );

    const celebrityMatches: CelebrityMatch[] = [];

    // Get best male match
    if (compatibleMales.length > 0) {
      const bestMale = compatibleMales[0];
      const compatibilityScore = this.calculateCelebrityCompatibility(
        userLifePath,
        userDestiny,
        userSoulUrge,
        bestMale.lifePathNumber
      );

      celebrityMatches.push({
        name: bestMale.name,
        birthDate: bestMale.birthDate,
        profession: bestMale.profession,
        lifePathNumber: bestMale.lifePathNumber,
        compatibilityScore,
        matchReason: this.generateFallbackMatchReason(
          userName,
          userLifePath,
        ...
        wikiLink: `https://en.wikipedia.org/wiki/${bestMale.name.replace(" ", "_")}`,
      });
    }

    // Get best female match
    if (compatibleFemales.length > 0) {
      const bestFemale = compatibleFemales[0];
      const compatibilityScore = this.calculateCelebrityCompatibility(
        userLifePath,
        userDestiny,
        userSoulUrge,
        bestFemale.lifePathNumber
      );

      celebrityMatches.push({
        name: bestFemale.name,
        birthDate: bestFemale.birthDate,
        profession: bestFemale.profession,
        lifePathNumber: bestFemale.lifePathNumber,
        compatibilityScore,
        matchReason: this.generateFallbackMatchReason(
          userName,
          userLifePath,
          bestFemale.name,
          bestFemale.lifePathNumber,
          bestFemale.profession
        ),
        wikiLink: `https://en.wikipedia.org/wiki/${bestFemale.name.replace(" ", "_")}`,
      });
    }

    return celebrityMatches.sort(
  ...
  static getIncompatibleNumbers(
    ...
    const incompatibleSoulUrge =
      this.getIncompatibleSoulUrgeNumbers(userSoulUrge);
  ...
  private static getIncompatibleDestinyNumbers(
  ...
  private static getIncompatibleSoulUrgeNumbers(
  ...
  private static calculateCelebrityCompatibility(
    ...
    if (userLifePath === celebrityLifePath) {
      ...
      const compatibilityBonus = this.getLifePathCompatibilityBonus(
        userLifePath,
        celebrityLifePath
      );
  ...
  private static generateFallbackMatchReason(
    ...
    const userTrait = compatibilityDescriptions[userLifePath as keyof typeof compatibilityDescriptions] || "unique";
    const celebrityTrait =
      compatibilityDescriptions[celebrityLifePath as keyof typeof compatibilityDescriptions] || "special";
  ...
  private static getLifePathCompatibilityBonus(
    lifePath1: number,
    lifePath2: number
  ): number {
